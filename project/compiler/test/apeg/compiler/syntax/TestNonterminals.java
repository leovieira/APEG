...
public class TestNonterminals {
	...
	public void peg_expr() throws RecognitionException, IOException {
		...
		String t2 = "'0' { x = 0;} / '1' { x = 1;} / x = .;"; // it must have a follow symbol
		parser = createParser(t2);
	...
	public void optReturn() throws RecognitionException, IOException {
		...
		tree = parser.optReturn().getTree();
		assertEquals(tree.toStringTree(), "(LIST (VARDECL Grammar g) (VARDECL int i) (VARDECL String s))");
	}
	...
	public void option() throws IOException, RecognitionException {
		...
		tree = parser.option().getTree();
		assertEquals(tree.toStringTree(), "(LIST ADAPTABLE NOT_ENV_DISCARDING MEMOIZATION)");
	}
	...
	public void number() throws IOException, RecognitionException {
		...
		// Test a real number - It has the integer and decimal part and also a positive exponent
		String t2 = "1.01e+2";
		parser = createParser(t2);
		...
		// Test a real number - It has the integer and decimal part and also a negative exponent
		String t3 = "2.12E-3";
		parser = createParser(t3);
		...
		// Test a real number - It has only the integer and exponent (negative) parts
		String t5 = "4.e-5";
		parser = createParser(t5);
	...
	public void ntcall() throws RecognitionException, IOException {
		...
		// Test a nonterminal call without attributes, but using a angular parenthesis
		String t2 = "nt<>";
		parser = createParser(t2);
		...
		tree = parser.ntcall().getTree();
		assertEquals(tree.toStringTree(), "(NONTERM nt (LIST (+ 1 2) a (OP_NOT (|| (< x y) (== z 2)))))");
	}
	...
	public void grammarDef() throws RecognitionException, IOException {
		// Test a grammar definition with an option and with a function declarations sections
		String t1 = "apeg g;"
		...
		assertEquals(tree.toStringTree(), "(GRAMMAR g (LIST ADAPTABLE NOT_ENV_DISCARDING MEMOIZATION) "
	...
	public void functions() throws RecognitionException, IOException {
		...
		assertEquals(tree.toStringTree(), "(FILES AdaptableFunctions FortressFunctions OtherFunctions)");
		
		// Test a list of non function declarations
		String t2 = "rule"; // It must have a follow. Here I put only a possible name of a rule
		parser = createParser(t2);
	...
	public void expr() throws IOException, RecognitionException {
		// Test an expression - An expression is a conditional expression. So, there is no much to do
	...
	public void designator() throws RecognitionException, IOException {
		...
		// Also, It is not allowing a function call as part of it, for the moment
		...
		tree = parser.designator().getTree();
		assertEquals(tree.toStringTree(), "(ARRAY_REF array (+ (* (CALL func (LIST true)) 2) 1))");
		...
		tree = parser.designator().getTree();
		assertEquals(tree.toStringTree(), "(ARRAY_REF (DOT (DOT (ARRAY_REF x i) z) y) j)");
	}
	...
	public void cond_expr() throws IOException, RecognitionException {
		...
		assertEquals(tree.toStringTree(), "(&& (>= x y) (OP_NOT (|| false (CALL func LIST))))");
		...
		tree = parser.condExpr().getTree();
		assertEquals(tree.toStringTree(), "(|| (|| (<= x y) (&& true (OP_NOT (CALL func LIST)))) (== z 2))");
	}
	...
	public void bool_expr() throws RecognitionException, IOException {
		...
		assertEquals(tree.toStringTree(), "(OP_NOT (&& (< x y) (CALL func (LIST (+ z 1)))))");
		...
		tree = parser.bool_expr().getTree();
		assertEquals(tree.toStringTree(), "(== (OP_NOT (&& (< x y) (CALL func (LIST (+ z 1))))) false)");
	}
	
	@Test
	public void attrORfunccall() throws IOException, RecognitionException {
		// Test of a attribute name
		String t1 = "x>"; // it must have a follow. In that case, the and of a nonterminal call
		parser = createParser(t1);
